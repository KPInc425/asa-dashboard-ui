...
const MapSelectionStep: React.FC<StepProps & { setCurrentStep?: (step: WizardStep) => void }> = ({ wizardData, setWizardData, availableMaps, toggleMap, updateMapCount, setCurrentStep }) => {
  ...
  React.useEffect(() => {
    if (wizardData.serverCount === 1 && availableMaps.length > 0) {
      // If no map is selected, auto-select the first available
      if (!wizardData.selectedMaps.some(m => m.enabled)) {
        const firstAvailable = availableMaps.find(m => m.available);
        if (firstAvailable) {
          setWizardData(prev => ({
            ...prev,
            selectedMaps: [{ map: firstAvailable.name, count: 1, enabled: true, displayName: firstAvailable.displayName }]
          }));
        }
      }
    }
  }, [wizardData.serverCount, availableMaps]);

  const isSingleServer = wizardData.serverCount === 1;
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-base-content/70 text-lg">
        {isSingleServer
          ? 'Select the map for your single server.'
          : 'Choose which maps you want to include in your cluster. You can select multiple maps and specify how many servers for each.'}
      </p>
              ...
              {isSingleServer ? (
                <input
                  ...
                  onChange={() => {
                    setWizardData(prev => ({
                      ...prev,
                      selectedMaps: [{ map: map.name, count: 1, enabled: true, displayName: map.displayName }]
                    }));
                  }}
      ...
      {/* Custom Map Input - hide for single-server clusters */}
      {!isSingleServer && (
