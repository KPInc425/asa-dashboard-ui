...
export const provisioningApi = {
   ...
   * Configure SteamCMD
   */
  configureSteamCmd: async (config: {
    customPath?: string;
    autoInstall?: boolean;
  }): Promise<{
    success: boolean;
    ...
    const response = await api.post(
      "/api/provisioning/configure-steamcmd",
      config,
    );
    return response.data;
  },
  ...
  installASABinaries: async (
    ...
    const response = await api.post("/api/provisioning/install-asa-binaries", {
      foreground,
    });
    return response.data;
  },
  ...
  getServerMods: async (
    ...
    success: boolean;
    serverConfig: { additionalMods: number[]; excludeSharedMods: boolean };
  }> => {
    if (FRONTEND_ONLY_MODE) {
      return new Promise((resolve) => {
        setTimeout(() => {
    ...
    const response = await api.get(
      `/api/provisioning/server-mods/${serverName}`,
    );
    return response.data;
  },
   ...
   */
  updateServerMods: async (
    serverName: string,
    config: { additionalMods: number[]; excludeSharedMods: boolean },
  ): Promise<{ success: boolean; message: string }> => {
    if (FRONTEND_ONLY_MODE) {
      return new Promise((resolve) => {
        setTimeout(() => {
   ...
   */
  getGlobalConfigs: async (): Promise<{
    success: boolean;
    gameIni: string;
   ...
   */
  getConfigExclusions: async (): Promise<{
    success: boolean;
   ...
   * Get mods overview
   */
  getModsOverview: async (): Promise<{
    success: boolean;
    overview: {
      sharedMods: number[];
      serverMods: Record<string, Record<string, unknown>>;
      totalServers: number;
    };
  }> => {
    if (FRONTEND_ONLY_MODE) {
        ...
        setTimeout(() => {
          resolve({
            ...
            overview: {
              ...
              serverMods: {
                MyServer1: {
                  additionalMods: [1609138312],
                  excludeSharedMods: false,
                },
                MyServer2: {
                  additionalMods: [215527665],
  ...
  getSystemLogs: async (
    ...
    const response = await api.get(
      `/api/provisioning/system-logs?${params.toString()}`,
    );
    return response.data;
  },
  ...
  getClusterDetails: async (
    clusterName: string,
  ): Promise<{ success: boolean; cluster: Record<string, unknown> }> => {
    if (useMockData()) {
      if (isDemoMode()) {
        const { getDemoClusters } = await import("../demo/demo-data");
        const clusters = getDemoClusters();
        ...
        return {
          success: true,
          cluster: (cluster || clusters[0]) as unknown as Record<
            string,
            unknown
          >,
        };
        ...
        setTimeout(() => {
          resolve({
            ...
            cluster: {
              ...
              servers: [
                {
                  name: `${clusterName}-Server1`,
                  gamePort: 7777,
                  status: "running",
                },
                {
                  name: `${clusterName}-Server2`,
                  gamePort: 7778,
                  status: "stopped",
                },
              ],
    ...
    const response = await api.get(
      `/api/provisioning/clusters/${encodeURIComponent(clusterName)}`,
    );
    return response.data;
  },
  ...
  startCluster: async (
    ...
    const response = await api.post(
      `/api/provisioning/clusters/${encodeURIComponent(clusterName)}/start`,
    );
    return response.data;
  },
  ...
  stopCluster: async (
    ...
    const response = await api.post(
      `/api/provisioning/clusters/${encodeURIComponent(clusterName)}/stop`,
    );
    return response.data;
  },
  ...
  restartCluster: async (
    ...
    const response = await api.post(
      `/api/provisioning/clusters/${encodeURIComponent(clusterName)}/restart`,
    );
    return response.data;
  },
  ...
  exportClusterConfig: async (clusterName: string): Promise<Blob> => {
    const response = await api.get(
      `/api/provisioning/clusters/${encodeURIComponent(clusterName)}/export`,
      { responseType: "blob" },
    );
    return response.data;
  },
   ...
   */
  downloadClusterBackup: async (
    clusterName: string,
    backupName: string,
  ): Promise<Blob> => {
    const response = await api.get(
      `/api/provisioning/clusters/${encodeURIComponent(clusterName)}/download-backup?backup=${encodeURIComponent(backupName)}`,
      { responseType: "blob" },
    );
    return response.data;
  },
   ...
   */
  downloadServerBackup: async (
    serverName: string,
    backupName: string,
  ): Promise<Blob> => {
    const response = await api.get(
      `/api/provisioning/servers/${encodeURIComponent(serverName)}/download-backup?backup=${encodeURIComponent(backupName)}`,
      { responseType: "blob" },
    );
    return response.data;
  },
  ...
  deleteServerBackup: async (
    ...
    const response = await api.delete(
      `/api/provisioning/servers/${encodeURIComponent(serverName)}/backups/${encodeURIComponent(backupName)}`,
    );
    return response.data;
  },
  ...
  restoreClusterBackup: async (
    ...
    const response = await api.post(
      `/api/provisioning/cluster-backups/${encodeURIComponent(clusterName)}/restore`,
      formData,
      {
        headers: { "Content-Type": "multipart/form-data" },
      },
  ...
  restoreServerBackup: async (
    ...
    const response = await api.post(
      `/api/provisioning/server-backups/${encodeURIComponent(serverName)}/restore`,
      formData,
      {
        headers: { "Content-Type": "multipart/form-data" },
      },
  ...
  getClusterBackups: async (
    ...
    const response = await api.get<{
      ...
      data: ClusterBackup[];
      message?: string;
    }>(`/api/provisioning/cluster-backups/${encodeURIComponent(clusterName)}`);
    if (!response.data.success) {
