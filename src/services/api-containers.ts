...
export const containerApi = {
  ...
  getContainers: async (): Promise<Container[]> => {
    if (useMockData()) {
      // In demo mode, use full mock data from demo-data module
      if (isDemoMode()) {
  ...
  startNativeServer: async (
    ...
    if (useMockData()) {
      ...
      return {
        success: true,
        message: `Native server ${name} started successfully`,
      };
    } else {
      try {
        if (!api) {
        ...
        if (error instanceof Error) {
          if (error.message.includes("timeout")) {
            throw new Error(
              `Server start request timed out. The server may still be starting in the background.`,
            );
  ...
  stopNativeServer: async (
    ...
    if (FRONTEND_ONLY_MODE) {
      ...
      return {
        success: true,
        message: `Native server ${name} stopped successfully`,
      };
    } else {
  ...
  restartNativeServer: async (
    ...
    if (FRONTEND_ONLY_MODE) {
      ...
      return {
        success: true,
        message: `Native server ${name} restarted successfully`,
      };
    } else {
  ...
  sendRconCommand: async (
    ...
    if (FRONTEND_ONLY_MODE) {
      ...
      const response =
        mockResponses[command.toLowerCase()] || `Command executed: ${command}`;
...
pause`,
    ...
    const response = await api.get(
      `/api/native-servers/${serverName}/start-bat`,
    );
    return response.data;
  },
   ...
   */
  regenerateNativeServerStartBat: async (
    serverName: string,
  ): Promise<{ success: boolean; message: string }> => {
    if (FRONTEND_ONLY_MODE) {
      return new Promise((resolve) => {
    ...
    const response = await api.post(
      `/api/native-servers/${serverName}/regenerate-start-bat`,
    );
    return response.data;
  },
  ...
  getConfigFile: async (
    ...
    try {
      const response = await api.get(
        `/api/native-servers/${serverName}/config/${fileName}`,
      );
      return response.data;
    } catch (error: unknown) {
      ...
      return {
        ...
        message:
          (error as { response?: { data?: { message?: string } } })?.response
            ?.data?.message || "Failed to update config file",
      };
    }
  },
  ...
  getServerLogs: async (
    ...
    try {
      const response = await api.get(
        `/api/native-servers/${encodeURIComponent(serverName)}/logs`,
        { params: options },
      );
      return response.data;
    } catch (error: unknown) {
      return {
        ...
        content:
          (error as { response?: { data?: { message?: string } } })?.response
            ?.data?.message || "Failed to get server logs",
      };
    }
  },
   ...
   */
  getAutoShutdownConfig: async (): Promise<{
    success: boolean;
    ...
    if (FRONTEND_ONLY_MODE) {
      return {
        ...
        config: {
          ...
          warningMessage:
            "Server will shut down in {time} minutes due to inactivity",
          excludeServers: [],
        },
      };
    } else {
   ...
   */
  updateAutoShutdownConfig: async (
    config: Record<string, unknown>,
  ...
  async getSaveFiles(
    ...
    try {
      const response = await api.get(
        `/api/native-servers/${encodeURIComponent(serverName)}/save-files`,
      );
      return response.data;
    } catch (error) {
  ...
  async uploadSaveFile(
    ...
    try {
      const response = await api.post(
        `/api/native-servers/${encodeURIComponent(serverName)}/save-files/upload`,
        formData,
        {
          headers: {
  ...
  async downloadSaveFile(
    ...
    try {
      const response = await api.get(
        `/api/native-servers/${encodeURIComponent(serverName)}/save-files/download/${encodeURIComponent(fileName)}`,
        {
          responseType: "blob",
        },
      );
