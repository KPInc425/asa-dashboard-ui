...
class SocketManager {
    ...
    setBaseUrl(url: string): void {
        ...
        // If currently connected, reconnect to new URL
    ...
    private resolveSocketUrl(): string {
        if (this._runtimeUrl) {
            return this._runtimeUrl.replace(/\/$/, "");
        }
    ...
    connect(containerName: string, logFile?: string): Promise<void> {
        return new Promise(async (resolve) => {
            ...
            const backendAvailable =
                await this.checkBackendAvailability(socketUrl);
            if (!backendAvailable) {
                console.warn(
                    "Backend server is not available, skipping Socket.IO connection",
                );
            ...
            this.socket.on("connect", async () => {
                console.log(
                    `✅ Socket.IO connected successfully for container: ${containerName}`,
                );
                ...
                if (this.reconnectAttempts < this.maxReconnectAttempts) {
                    ...
                    setTimeout(() => {
                        if (this.containerName) {
                            this.connect(this.containerName).catch(
                                console.error,
                            );
                        }
                    }, this.reconnectDelay);
    ...
    switchLogFile(logFile: string): void {
        if (this.socket?.connected && this.containerName) {
            ...
            this.socket.emit("start-ark-logs", {
                serverName: this.containerName,
                logFileName: logFile,
            });
        }
    }
    ...
    switchToContainerLogs(): void {
        if (this.socket?.connected && this.containerName) {
            ...
            this.socket.emit("start-container-logs", {
        ...
        if (this.socket) {
            this.socket.on("ark-log-data", (data) => {
                ...
                const logMessage: LogMessage = {
                    ...
                    container:
                        data.container || this.containerName || "unknown",
                };
                callback(logMessage);
            });
        }
        ...
        return new Promise((resolve) => {
            ...
            this.socket.on("connect", async () => {
                ...
                try {
                    ...
                    console.error(
                        "Error in system logs connection setup:",
                        error,
                    );
                    resolve();
                }
            ...
            this.socket.on("connect_error", (error: Error) => {
                ...
                // Instead of rejecting, resolve with a warning to allow the app to continue
                console.warn(
                    "System logs Socket.IO connection failed, using static logs only:",
                    error,
                );
                resolve(); // Resolve without error to allow the app to continue
            });
            ...
            this.socket.on("disconnect", (reason: string) => {
                ...
                if (this.reconnectAttempts < this.maxReconnectAttempts) {
                    this.reconnectAttempts++;
                    console.log(
                        `Attempting to reconnect system logs (${this.reconnectAttempts}/${this.maxReconnectAttempts})...`,
                    );
                    ...
                    console.error(
                        "Max reconnection attempts reached for system logs",
                    );
                }
            });
        });
