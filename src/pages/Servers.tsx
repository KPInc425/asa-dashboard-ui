...
const Servers: React.FC = () => {
  ...
  // Use the adapter-driven inventory services with legacy fallback
  const inventoryQuery = useServices();
  const legacyQuery = useLegacyServers({
    refetchInterval: 10_000,
    enabled: !!inventoryQuery.error, // Fall back to legacy on adapter failure
  });

  // Use adapter data when available, fall back to legacy
  const {
    data: serversData,
    isLoading: loading,
    error: queryError,
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-sm md:text-base text-base-content/70">
                Manage your ARK: Survival Ascended servers (containers & native)
              </p>
            </div>
              ...
              <div className="flex gap-2">
                {isDeveloperMode && supportsCapability("canViewStatus") && (
                  <>
                    ...
                    <button
                      onClick={() =>
                        navigate("/provisioning?action=debug-clusters")
                      }
          ...
          {servers.length === 0 ? (
            <div className="text-center py-8 md:py-12">
              ...
              <p className="text-sm text-base-content/50">
                Start by creating your first ARK server
              </p>
            </div>
          ) : layoutMode === "cards" ? (
            // Card Layout
