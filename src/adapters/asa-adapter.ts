...
// Demo mode interceptor for adapter-based data flow
...
export class ASAAdapter implements BackendAdapter {
  ...
  constructor(binding: BackendBinding, environmentId?: string) {
    ...
    // Install demo mode interceptor so adapter-based data flow
    // (useServices, useService, etc.) returns mock data in demo mode.
  ...
  async connect(): Promise<void> {
    ...
    try {
      await this.httpClient.get(this.binding.healthEndpoint ?? "/health", {
        timeout: 5_000,
      });
  ...
  async listServices(): Promise<ServiceEntry[]> {
    ...
    const nativeServers = (nativeServersRes.data.servers ?? []).map((s: any) =>
      this._toServiceEntry(s, "native-server"),
    );
  ...
  async getServiceHealth(serviceId: string): Promise<HealthStatusData> {
    ...
    try {
      ...
      return {
        serviceId,
        health: statusData.status === "running" ? "healthy" : "unhealthy",
        lastCheckedAt: Date.now(),
      };
    } catch {
      return {
      ...
      switch (action.actionId) {
        case "start": {
          ...
          return {
            ...
            message: startRes.data.message ?? `Server "${localId}" started`,
            completedAt,
          };
        }
        ...
        case "stop": {
          ...
          return {
            ...
            message: stopRes.data.message ?? `Server "${localId}" stopped`,
            completedAt,
          };
        }
